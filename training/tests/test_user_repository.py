...
def test_get_report_01_no_filters(user_repo_with_data: UserRepository):
    """
    Test fetching report data without any filters applied.
    """
    report_filter = schemas.Report01Filter()
    results = user_repo_with_data.get_report_01(report_filter)
...
def test_get_report_01_filter_by_agency(user_repo_with_data: UserRepository):
    """
    Test fetching report data with agency filter.
    """
    agency_id = 1  # "Department of Mysteries"
    report_filter = schemas.Report01Filter(agency_id=agency_id)
    results = user_repo_with_data.get_report_01(report_filter)
...
def test_get_report_01_filter_by_bureau(user_repo_with_data: UserRepository):
    """
    Test fetching report data with bureau filter.
    """
    bureau_id = 2  # Assuming bureau_id of 2 corresponds to a specific bureau
    report_filter = schemas.Report01Filter(bureau_id=bureau_id)
    results = user_repo_with_data.get_report_01(report_filter)
...
def test_get_report_01_filter_by_date_range(user_repo_with_data: UserRepository):
    """
    Test fetching report data with completion date range filter.
    """
    start_date = datetime(2023, 1, 1)
    end_date = datetime(2024, 12, 31)
    report_filter = schemas.Report01Filter(completion_date_start=start_date, completion_date_end=end_date)
    results = user_repo_with_data.get_report_01(report_filter)
...
def test_get_report_01_filter_by_quiz_name(user_repo_with_data: UserRepository):
    """
    Test fetching report data by filtering with specific quiz names.
    """
    quiz_names = ["Travel Training for Ministry of Magic"]
    report_filter = schemas.Report01Filter(quiz_names=quiz_names)
    results = user_repo_with_data.get_report_01(report_filter)
...
def test_get_report_01_combined_filters(user_repo_with_data: UserRepository):
    """
