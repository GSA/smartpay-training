...
def auth_exchange(
    ...
    if not db_user:
        logging.info(
            f"Token exchange attempt: UAA authenticated, but not found in database: {uaa_user['email']}"
        )
        raise HTTPException(
    ...
    if not user.is_admin():
        logging.info(f"Token exchange attempt: UAA authenticated, but not an admin: {uaa_user['email']}")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    logging.info(f"Token exchange success: {uaa_user['email']}")
