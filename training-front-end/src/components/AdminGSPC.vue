      ...
      <li class="usa-card tablet:grid-col-12">
        <div class="usa-card__container">
          ...
          <div class="usa-card__body">
            ...
            <form ref="form" class="usa-form usa-form--large margin-bottom-3" data-test="gspc-form"
              @submit.prevent="submitGspcInvites">
              <ValidatedTextArea v-model="user_input.emailAddresses" client:load :validator="v_all_info$.emailAddresses"
                label="Email Addresses of GSA SmartPay Forum Attendees" name="email-list" :required="true" />
              <ValidatedMemorableDatepicker v-model="user_input.certificationExpirationDate" client:load
                :validator="v_all_info$.certificationExpirationDate" label="Certification Expiration Date"
                name="certification-expiration-date" hint-text="For example: January 19 2000" :required="true" />
              <div>
                <USWDSAlert v-if="showFailedMessage" status="error" class="usa-alert--slim" :has-heading="false">
                  Emails failed to send to: {{ failedEmailList }}
                </USWDSAlert>
                <USWDSAlert v-if="showSuccessMessage" status="success" class="usa-alert--slim" :has-heading="false">
                  Emails sending to {{ successCount }} people.
                </USWDSAlert>
              </div>
              <div class="grid-row grid-gap margin-top-3">
                <div class="grid-col">
                  <input class="usa-button" type="submit" value="Send Invitations" :disabled="isLoading"
                    data-test="submit">
                  <button id="cancel" type="button" class="usa-button usa-button--outline" :disabled="isLoading"
                    @click="cancel">
                    Cancel
                  </button>
                </div>
      ...
      <li class="usa-card tablet:grid-col-12">
        <div class="usa-card__container">
          ...
          <div class="usa-card__body">
            ...
            <div>
              <USWDSAlert v-if="showFollowUpSuccessMessage" status="success" class="usa-alert--slim"
                :has-heading="false">
            ...
            <div class="grid-row grid-gap margin-top-3">
              <div class="grid-col">
                <button id="send-out-follow-ups" type="button" class="usa-button" :disabled="isLoading"
